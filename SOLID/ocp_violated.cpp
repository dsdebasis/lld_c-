...
class ShopingCartStorage{
    ...
    void saveToMySql(){
        cout << "Saving the shoping cart to MySql db" << endl;
    }
    void saveToMongoDb(){
        cout << "Saving the shoping cart to MongoDb db" << endl;
    }
    void saveToRedis(){
        cout << "Saving the shoping cart to redis db" << endl;
    }
...
int main(){
    ...
    //one class has multiple resaons to change (Single responsible violated)
    // we have to chage to the parent class to add new db
    // this is agian violates the Open Closed Principle
    db->saveToMySql();
    db->saveToMongoDb();
    db->saveToRedis();
